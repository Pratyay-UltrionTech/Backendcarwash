...
def _operating_range_minutes(open_time: str, close_time: str) -> tuple[int, int]:
    ...
    close_raw = parse_time_to_minutes(close_time)
...
def list_available_slots(
    ...
    *,
    ...
    for t in range(open_m, close_m - dur + 1, BASE_SLOT_MINUTES):
        ...
        if date_iso == today_iso and t < now_minutes:
            continue

        st = format_minutes_to_hhmm(t)
        et = format_minutes_to_hhmm(t + dur)
        available_bays = count_bays_available_for_start_duration(
        ...
        out.append(
            {
                ...
                "capacity": schedule_open,
                "booked": max(0, schedule_open - available_bays),
