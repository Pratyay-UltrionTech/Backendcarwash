...
from app.services.duration_slots import (
    BASE_SLOT_MINUTES,
    resolve_operating_day_end_minutes,
...
def _driver_can_service_pin(driver: MobileServiceDriver, requested_pin: str | None, strict: bool = False) -> bool:
    """Return True if driver may take a job at requested_pin.

    strict=True (used for user-facing booking): a driver with no explicit serviceable zip list
    does NOT match any specific zip — only drivers with that zip explicitly configured are eligible.
    strict=False (legacy / manager-portal): empty list means no restriction within the manager's area.
    """
    ...
    if _normalize_pin(driver.service_pin_code) == pin:
        return True
    zips = {_normalize_pin(z) for z in loads_json_array(driver.serviceable_zip_codes_json) if str(z).strip()}
    if not zips:
        return not strict
    return pin in zips
...
def is_driver_available_for_interval(
    ...
    *,
    ...
    if target_driver is None:
        return False
    if not _driver_can_service_pin(target_driver, requested_pin, strict=strict_zip):
        return False
    ...
    bookings = (
        ...
        .filter(
            ...
            MobileBooking.status.notin_(["cancelled", "completed", "rejected", "failed"]),
        )
        .all()
    )
    ...
    open_m = _parse_hhmm(slot_settings.open_time if slot_settings else "09:00")
    close_raw = _parse_hhmm(slot_settings.close_time if slot_settings else "17:00")
...
def allocate_driver_for_interval(
    ...
    *,
    ...
    for d in active_drivers:
        if not _driver_can_service_pin(d, requested_pin, strict=strict_zip):
            continue
...
def list_slot_availability(
    ...
    *,
    ...
    # For user-facing searches, only count drivers who can service the requested zip.
    zip_eligible_drivers = [d for d in active_drivers if _driver_can_service_pin(d, requested_pin, strict=strict_zip)]

    dur = snap_duration_to_base_slots(int(booking_duration_minutes or BASE_SLOT_MINUTES))
    slots_n = slots_needed_for_duration(dur)
    open_time = _parse_hhmm(slot_settings.open_time if slot_settings else "09:00")
    close_raw = _parse_hhmm(slot_settings.close_time if slot_settings else "17:00")
    close_time = resolve_operating_day_end_minutes(open_time, close_raw)

    total_d = len(zip_eligible_drivers)
    ...
    while t + dur <= close_time:
        ...
        if slot_date == today_iso and t < now_minutes:
            t += BASE_SLOT_MINUTES
            continue
        ...
        eligible = [
            d.id
            for d in zip_eligible_drivers
            if is_driver_available_for_interval(db, manager, slot_date, st, et, d.id, requested_pin=requested_pin, strict_zip=strict_zip)
        ]
        out.append(
            SlotAvailability(
                start_time=st,
...
def assert_slot_available(
    ...
    *,
    ...
    if allocate_driver_for_interval(db, manager, slot_date, start_time, end_time, requested_pin=requested_pin, strict_zip=strict_zip) is None:
...
def assert_driver_assignable(
    ...
    if not _driver_can_service_pin(d, requested_pin, strict=True):
...
def manager_for_service_pin(
    ...
    for m in managers:
        ...
        count = 0
        for d in drivers:
            zips = {_normalize_pin(z) for z in loads_json_array(d.serviceable_zip_codes_json) if str(z).strip()}
            if pin == _normalize_pin(d.service_pin_code) or pin in zips:
                count += 1
        if count > best_count:
            best = m
            best_count = count
