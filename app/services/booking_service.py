...
def washer_ids_busy_in_slot(
    ...
    q = db.query(BranchBooking).filter(
        ...
        BranchBooking.status.notin_(["cancelled", "completed", "rejected", "failed"]),
...
def create_booking(
    ...
    *,
    ...
    notes: str = "",
    tip_cents: int = 0,
    service_charged_cents: int | None = None,
    ...
    if slot_date < today_iso:
        raise AppError("Cannot book a past date", code="past_date", status_code=400)
    
    if slot_date == today_iso:
        start_m = slot_service.parse_time_to_minutes(start_time)
        now_m = now.hour * 60 + now.minute
        if start_m < now_m:
            raise AppError("Cannot book a past time", code="past_time", status_code=400)
    ...
    from app.services.booking_pricing import branch_booking_catalog_package_cents, resolve_charged_service_cents

    bid = _parse_client_booking_id(booking_id)
    cust_email = customer_email.strip() if isinstance(customer_email, str) else ""
    if len(cust_email) > 320:
        cust_email = cust_email[:320]
    ...
    db.add(job)
    db.flush()
    pkg = branch_booking_catalog_package_cents(db, job)
    charged, promo_disc = resolve_charged_service_cents(pkg, service_charged_cents)
    job.service_charged_cents = charged
    job.promo_discount_cents = promo_disc
...
def patch_branch_booking_fields(db: Session, branch: Branch, job: BranchBooking, data: dict[str, Any]) -> None:
    ...
    from app.services.booking_status import assert_schedule_and_staff_editable

    assert_schedule_and_staff_editable(
        job.status,
        data,
        current_slot=(job.slot_date, job.start_time, job.end_time),
        current_staff_id=job.assigned_washer_id,
        current_bay=job.bay_number,
        staff_field="assigned_washer_id",
    )
    ...
    if any(k in data for k in slot_keys):
        ...
        slot_changed = (sd, st, et) != (job.slot_date, job.start_time, job.end_time)

        if slot_changed:
            # Validate that the new slot is in the future only when the booking is
            # actually being re-scheduled, not when clients PATCH unchanged slot fields.
            now = datetime.now()
            today_iso = now.date().isoformat()
            if sd < today_iso:
                raise AppError("Cannot reschedule to a past date", code="past_date", status_code=400)
            if sd == today_iso:
                start_m = slot_service.parse_time_to_minutes(st)
                now_m = now.hour * 60 + now.minute
                if start_m < now_m:
                    raise AppError("Cannot reschedule to a past time slot", code="past_time", status_code=400)
    ...
    if "vehicle_model" in data and data["vehicle_model"] is not None:
        job.vehicle_model = str(data["vehicle_model"]).strip()
    if "registration_number" in data and data["registration_number"] is not None:
    ...
    if "assigned_washer_id" in data:
        wid = data["assigned_washer_id"]
        if isinstance(wid, str) and not wid.strip():
            wid = None
        assign_washer(db, job, wid)
        # Auto-advance to assigned when a washer is set and status is still scheduled.
        if job.assigned_washer_id and job.status == "scheduled" and "status" not in data:
            job.status = "assigned"
    ...
    if "status" in data and data["status"] is not None:
        ...
        prev_st = job.status
        job.status = st
        _actor_id = str(data.get("_actor_id") or "")
        _actor_role = str(data.get("_actor_role") or "manager")
        if _actor_id:
            job.updated_by = _actor_id
            job.updated_by_role = _actor_role
        _now = datetime.now(timezone.utc)
        if st == "cancelled" and prev_st != "cancelled":
            job.cancelled_at = _now
            if _actor_id:
                job.cancelled_by = _actor_id
        elif st != "cancelled":
