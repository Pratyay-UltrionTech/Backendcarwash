...
def list_customers(
    ...
    sort_by: str = Query(default="recently_created_booking"),
    ...
    if type_filter in ("all", "account"):
        ...
        for u in users:
            ...
            last_bc = db.query(BranchBooking.created_at).filter(BranchBooking.customer_id == u.id).order_by(BranchBooking.created_at.desc()).first()
            last_mc = db.query(MobileBooking.created_at).filter(MobileBooking.customer_id == u.id).order_by(MobileBooking.created_at.desc()).first()
            last_created_times = [d[0] for d in [last_bc, last_mc] if d and d[0]]
            last_booking_created_at = max(last_created_times).isoformat() if last_created_times else None
            ...
            customers.append({
                ...
                "last_booking_created_at": last_booking_created_at,
    ...
    if type_filter in ("all", "guest"):
        ...
        for name, phone, email, vtype, sdate, cat in branch_guests:
            ...
            if cat:
                g["created_ats"].append(cat)

        for name, phone, email, vtype, sdate, cat in mobile_guests:
            key = (phone or "").strip() or (name or "").strip()
            if not key:
                continue
            g = groups[key]
        ...
        for key, g in groups.items():
            ...
            customers.append({
                ...
                "last_booking_created_at": max(g["created_ats"]).isoformat() if g["created_ats"] else None,
                "created_at": None,
            })
    ...
    elif sort_by == "recently_created_booking":
        # Most-recently created booking first (creation timestamp, not slot date).
        customers.sort(key=lambda c: (c["name"] or "").lower())  # secondary: name A→Z
        customers.sort(key=lambda c: c["last_booking_created_at"] or "", reverse=True)
    else:  # recent_booking
        # Most-recent last-booking slot date first; ties broken by name A→Z.
        customers.sort(key=lambda c: (c["name"] or "").lower())  # secondary: name A→Z
...
def get_customer_bookings(
    ...
    branch_rows = bq.order_by(BranchBooking.created_at.desc()).all()
    ...
    mobile_rows = mq.order_by(MobileBooking.created_at.desc()).all()
    ...
    if mobile_rows:
        ...
        mobile_service_ids = {b.service_id for b in mobile_rows if b.service_id}
        mobile_svc_map = (
            {s.id: s.name for s in db.query(MobileCatalogServiceItem).filter(MobileCatalogServiceItem.id.in_(mobile_service_ids)).all()}
            if mobile_service_ids
            else {}
        )
        ...
        for b in mobile_rows:
            ...
            try:
                ...
                service_total_cents = 0
            service_label = mobile_svc_map.get(b.service_id or "", "") if b.service_id else ""
            if not service_label.strip():
                service_label = (b.notes or "").strip()
            bookings.append({
                "source": "mobile",
                "id": b.id,
                "created_at": b.created_at.isoformat() if b.created_at else "",
...
# ──────────────────────────────────────────────
# Revenue Report (new — detailed KPIs + chart)
# ──────────────────────────────────────────────

@router.get("/reports/revenue-detail")
def get_revenue_detail_report(
    db: DbSession,
    _admin: AdminUser,
    request: Request,
    ...
    channel: str = Query("all", description="all | branch | mobile"),
    payment_method: str = Query("all", description="all | pay_after | card | apple_pay"),
) -> dict[str, Any]:
    """Detailed revenue KPIs + daily bar-chart data for the Revenue Summary report.
    Returns: total_revenue, net_revenue, total_discount, tips_collected,
             avg_per_booking, booking_count, chart (list of daily buckets).
    All monetary values are in dollars (float, 2 dp).
    """
    ...
    # Cap range to 366 days to avoid oversized queries
    if (d_to - d_from).days > 366:
        d_to = d_from + _td(days=366)

    pm_filter = None if payment_method == "all" else payment_method.lower()

    # ── Accumulate per-day buckets ───────────────────────────────────────
    # key = ISO date string, value = {gross, net, discount, tips, count}
    daily: dict[str, dict] = {}
    cur = d_from
    while cur <= d_to:
        daily[cur.isoformat()] = {"gross": 0.0, "net": 0.0, "discount": 0.0, "tips": 0.0, "count": 0}
        cur += _td(days=1)

    def _accumulate(bookings_iter):
        """Add each booking's revenue into the correct daily bucket."""
        for b in bookings_iter:
            # Use completed_at date if available, else slot_date
            if getattr(b, "completed_at", None):
                day_key = b.completed_at.date().isoformat()
            else:
                day_key = b.slot_date
            ...
            # Monetary fields stored in cents → convert to dollars
            charged = (getattr(b, "service_charged_cents", None) or 0) / 100
            disc = (getattr(b, "promo_discount_cents", None) or 0) / 100
            tip = (getattr(b, "tip_cents", None) or 0) / 100

            # gross = what customer paid (net) + discount given
            net = max(0.0, charged)
            gross = net + disc

            daily[day_key]["gross"] += gross
            daily[day_key]["net"] += net
            daily[day_key]["discount"] += disc
            daily[day_key]["tips"] += tip
            daily[day_key]["count"] += 1

    # ── Branch bookings ──────────────────────────────────────────────────
    if channel in ("all", "branch"):
        bq = db.query(BranchBooking).filter(
            BranchBooking.slot_date >= date_from,
            BranchBooking.slot_date <= date_to,
            BranchBooking.status.notin_(["cancelled", "rejected", "failed"]),
        )
        if pm_filter:
            bq = bq.filter(BranchBooking.payment_method == pm_filter)
        _accumulate(bq.all())

    # ── Mobile bookings ──────────────────────────────────────────────────
    if channel in ("all", "mobile"):
        mq = db.query(MobileBooking).filter(
            MobileBooking.slot_date >= date_from,
            MobileBooking.slot_date <= date_to,
            MobileBooking.status.notin_(["cancelled", "rejected", "failed"]),
        )
        if pm_filter:
            mq = mq.filter(MobileBooking.payment_method == pm_filter)
        _accumulate(mq.all())

    # ── Aggregate KPIs ───────────────────────────────────────────────────
    total_gross = round(sum(v["gross"] for v in daily.values()), 2)
    total_net = round(sum(v["net"] for v in daily.values()), 2)
    total_discount = round(sum(v["discount"] for v in daily.values()), 2)
    total_tips = round(sum(v["tips"] for v in daily.values()), 2)
    total_count = sum(v["count"] for v in daily.values())
    avg_per_booking = round(total_net / total_count, 2) if total_count > 0 else 0.0

    # ── Chart data ───────────────────────────────────────────────────────
    # For ranges > 60 days collapse to weekly buckets to keep chart readable
    num_days = (d_to - d_from).days + 1
    if num_days <= 60:
        chart = [
            {
                "label": k,
                "total_revenue": round(v["gross"], 2),
        ...
        # Weekly buckets (ISO week, Mon–Sun)
        weekly: dict[str, dict] = {}
        for k, v in daily.items():
            week_start = _date.fromisoformat(k)
            week_start -= _td(days=week_start.weekday())  # Monday of that week
            wk = week_start.isoformat()
            if wk not in weekly:
                weekly[wk] = {"gross": 0.0, "net": 0.0, "discount": 0.0, "tips": 0.0, "count": 0}
            for field in ("gross", "net", "discount", "tips", "count"):
                weekly[wk][field] += v[field]
        chart = [
            {
                "label": k,
                "total_revenue": round(v["gross"], 2),
    ...
    action_log(
        "admin_get_revenue_detail_report",
        "success",
        request,
        channel=channel,
...
@router.get("/reports/booking-detail")
def get_booking_detail_report(
    db: DbSession,
    _admin: AdminUser,
    request: Request,
    date_from: str = Query(..., description="Start date inclusive (YYYY-MM-DD)"),
    date_to: str = Query(..., description="End date inclusive (YYYY-MM-DD)"),
    channel: str = Query("all", description="all | branch | mobile"),
    customer_type: str = Query("all", description="all | account | guest"),
    service_type: str = Query("all", description="all | <service name>"),
) -> dict[str, Any]:
    """Booking summary KPIs + daily bar-chart data for the Booking Summary report.
    Returns: total, completed, cancelled, rescheduled counts + chart list.
    """
    from datetime import date as _date, timedelta as _td
    ...
    cur = d_from
    while cur <= d_to:
        daily[cur.isoformat()] = {"total": 0, "completed": 0, "cancelled": 0, "rescheduled": 0, "in_progress": 0}
        cur += _td(days=1)

    CANCELLED_STATUSES = {"cancelled", "rejected", "failed"}
    COMPLETED_STATUSES = {"completed"}
    RESCHEDULED_STATUSES = {"rescheduled"}
    ...
    def _accumulate(bookings_iter):
        for b in bookings_iter:
            ...
            elif st in RESCHEDULED_STATUSES:
                daily[day_key]["rescheduled"] += 1
            else:
                daily[day_key]["in_progress"] += 1
    ...
    if channel in ("all", "branch"):
        ...
        if customer_type == "account":
            bq = bq.filter(BranchBooking.customer_id.isnot(None))
        elif customer_type == "guest":
            bq = bq.filter(BranchBooking.customer_id.is_(None))
        if service_type and service_type != "all":
            bq = bq.join(CatalogServiceItem, BranchBooking.service_id == CatalogServiceItem.id, isouter=True)
            bq = bq.filter(CatalogServiceItem.name == service_type)
        _accumulate(bq.all())
    ...
    if channel in ("all", "mobile"):
        ...
        if customer_type == "account":
            mq = mq.filter(MobileBooking.customer_id.isnot(None))
        elif customer_type == "guest":
            mq = mq.filter(MobileBooking.customer_id.is_(None))
        if service_type and service_type != "all":
            mq = mq.join(MobileCatalogServiceItem, MobileBooking.service_id == MobileCatalogServiceItem.id, isouter=True)
            mq = mq.filter(MobileCatalogServiceItem.name == service_type)
        _accumulate(mq.all())

    # ── KPI totals ───────────────────────────────────────────────────────
    total = sum(v["total"] for v in daily.values())
    completed = sum(v["completed"] for v in daily.values())
    cancelled = sum(v["cancelled"] for v in daily.values())
    rescheduled = sum(v["rescheduled"] for v in daily.values())
    in_progress = sum(v["in_progress"] for v in daily.values())
    ...
    if num_days <= 60:
        ...
        for k, v in daily.items():
            week_start = _date.fromisoformat(k)
            week_start -= _td(days=week_start.weekday())
            wk = week_start.isoformat()
            if wk not in weekly:
                weekly[wk] = {"total": 0, "completed": 0, "cancelled": 0, "rescheduled": 0, "in_progress": 0}
            for field in ("total", "completed", "cancelled", "rescheduled", "in_progress"):
                weekly[wk][field] += v[field]
        chart = [
            {
                "label": k,
    ...
    action_log(
        "admin_get_booking_detail_report",
        "success",
        request,
        channel=channel,
...
@router.get("/reports/staff-jobs")
def get_staff_jobs_report(
    db: DbSession,
    _admin: AdminUser,
    request: Request,
    date_from: str = Query(..., description="Start date inclusive (YYYY-MM-DD)"),
    date_to: str = Query(..., description="End date inclusive (YYYY-MM-DD)"),
    staff_type: str = Query("all", description="all | branch | mobile"),
) -> dict[str, Any]:
    """Staff job completion KPIs + per-staff bar chart.
    Returns: total_completed, avg_per_staff, total_tips, staff_count, chart list.
    Tips in dollars (float). Chart sorted by completed descending.
    """
    from datetime import date as _date, timedelta as _td
    ...
    # staff_id → {name, completed, tips_cents, staff_type}
    staff_map: dict[str, dict] = {}

    # ── Branch washers ───────────────────────────────────────────────────
    if staff_type in ("all", "branch"):
        bq = (
            db.query(BranchBooking)
            .filter(
                BranchBooking.slot_date >= date_from,
                BranchBooking.slot_date <= d_to.isoformat(),
                BranchBooking.status == "completed",
                BranchBooking.assigned_washer_id.isnot(None),
            )
            .all()
        )
        # Resolve washer names in bulk
        washer_ids = {b.assigned_washer_id for b in bq if b.assigned_washer_id}
        washer_names: dict[str, str] = {}
        if washer_ids:
            from app.models import Washer as WasherModel
            rows = db.query(WasherModel).filter(WasherModel.id.in_(washer_ids)).all()
            washer_names = {
                w.id: (w.name.strip() or w.login_id or w.id) for w in rows
            }
        for b in bq:
            wid = b.assigned_washer_id
            if not wid:
                continue
            if wid not in staff_map:
                staff_map[wid] = {
                    "name": washer_names.get(wid, wid),
                    "completed": 0,
                    "tips_cents": 0,
                    "staff_type": "branch",
                }
            staff_map[wid]["completed"] += 1
            staff_map[wid]["tips_cents"] += int(getattr(b, "tip_cents", 0) or 0)

    # ── Mobile drivers ───────────────────────────────────────────────────
    if staff_type in ("all", "mobile"):
        mq = (
            db.query(MobileBooking)
            .filter(
                MobileBooking.slot_date >= date_from,
                MobileBooking.slot_date <= d_to.isoformat(),
                MobileBooking.status == "completed",
                MobileBooking.assigned_driver_id.isnot(None),
            )
            .all()
        )
        driver_ids = {b.assigned_driver_id for b in mq if b.assigned_driver_id}
        driver_names: dict[str, str] = {}
        if driver_ids:
            from app.models.mobile import MobileServiceDriver
            rows = db.query(MobileServiceDriver).filter(MobileServiceDriver.id.in_(driver_ids)).all()
            driver_names = {
                d.id: ((d.emp_name or "").strip() or (getattr(d, "login_id", "") or "").strip() or d.id)
                for d in rows
            }
        for b in mq:
            did = b.assigned_driver_id
            if not did:
                continue
            ...
            staff_map[did]["completed"] += 1
            staff_map[did]["tips_cents"] += int(getattr(b, "tip_cents", 0) or 0)

    # ── KPI totals ───────────────────────────────────────────────────────
    total_completed = sum(v["completed"] for v in staff_map.values())
    total_tips = round(sum(v["tips_cents"] for v in staff_map.values()) / 100, 2)
    staff_count = len(staff_map)
    avg_per_staff = round(total_completed / staff_count, 2) if staff_count > 0 else 0.0

    # ── Chart: per-staff, sorted by completed desc ───────────────────────
    chart = sorted(
        [
            {
                "staff_id": sid,
                "name": v["name"],
                "completed": v["completed"],
                "tips": round(v["tips_cents"] / 100, 2),
                "staff_type": v["staff_type"],
            }
            for sid, v in staff_map.items()
        ],
