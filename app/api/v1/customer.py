...
def reschedule_booking(
    ...
    if job:
        ...
        prev_date, prev_start = job.slot_date, job.start_time
        job.slot_date = sd
        job.start_time = st
        job.end_time = et
        try:
            ...
            from app.services.email_service import send_booking_rescheduled_email, send_staff_booking_notification
            if u.email:
            ...
            send_staff_booking_notification(
                ...
                old_slot_date=prev_date,
                old_start_time=prev_start,
                customer_id=str(job.customer_id) if job.customer_id else None,
            )
    ...
    if mjob:
        ...
        if not owns_m and normalize_phone(mjob.phone) != phone:
            raise HTTPException(status_code=403, detail={"detail": "Forbidden", "code": "forbidden"})
        prev_date_m, prev_start_m = mjob.slot_date, mjob.start_time
        mjob.slot_date = sd
        mjob.start_time = st
        mjob.end_time = et
        try:
            ...
            if u.email:
                send_booking_rescheduled_email(
                    ...
                    booking_id=mjob.id,
                    customer_id=str(mjob.customer_id) if mjob.customer_id else None,
                    phone=mjob.phone or None,
                    new_end_time=mjob.end_time or None,
                    service_summary=getattr(mjob, "vehicle_summary", "") or "",
                    channel="mobile",
                )
            send_staff_booking_notification(
                db,
                ...
                booking_type="mobile",
                booking_id=mjob.id,
                customer_name=mjob.customer_name or "",
                phone=mjob.phone or "",
                vehicle_type=mjob.vehicle_type or "",
                vehicle_model=mjob.vehicle_model or "",
                registration_number=mjob.registration_number or "",
                service_summary=getattr(mjob, "vehicle_summary", "") or "",
                slot_date=mjob.slot_date,
                start_time=mjob.start_time,
                end_time=mjob.end_time,
                city_pin_code=str(mjob.city_pin_code),
                old_slot_date=prev_date_m,
                old_start_time=prev_start_m,
                customer_id=str(mjob.customer_id) if mjob.customer_id else None,
            )
