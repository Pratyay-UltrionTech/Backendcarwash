...
def create_booking(
    ...
    assigned_driver_id = (body.assigned_driver_id or "").strip() or None
    source = (body.source or "online").strip().lower()
    manager_created = source in ("walk_in", "manager", "phone")
    if manager_created and not assigned_driver_id:
        raise HTTPException(
            status_code=400,
            detail={"detail": "Select a driver before creating this booking.", "code": "driver_required"},
        )
    ...
    row = MobileBooking(
        ...
        status="assigned" if assigned_driver_id else "scheduled",
        source=body.source,
        notes=body.notes,
...
def patch_booking(
    ...
    assert_schedule_and_staff_editable(
        row.status,
        data,
        current_slot=(row.slot_date, row.start_time, row.end_time),
        current_staff_id=row.assigned_driver_id,
        staff_field="assigned_driver_id",
    )
    ...
    if any(k in data for k in ("slot_date", "start_time", "end_time")):
        ...
        slot_changed = (sd, st, et) != (row.slot_date, row.start_time, row.end_time)
        ...
        if slot_changed:
            ...
            if sd < today_iso:
                raise HTTPException(
                    status_code=400,
                    detail={"detail": "Cannot reschedule to a past date", "code": "past_date"},
                )
            if sd == today_iso:
                current_time = now.strftime("%H:%M")
                if st < current_time:
                    raise HTTPException(
                        status_code=400,
                        detail={"detail": "Cannot reschedule to a past time slot", "code": "past_time"},
                    )

            try:
                mobile_slot_service.assert_slot_available_for_booking_update(
                    db, manager_row, sd, st, et, row.id
                )
            except ValueError as e:
                code = str(e)
                if code == "slot_unavailable":
                ...
                if code == "driver_not_open":
                    raise HTTPException(
                        status_code=409,
                        detail={"detail": "Driver is not available for this slot", "code": "driver_unavailable"},
                    )
                if code == "driver_busy":
                    raise HTTPException(
                        status_code=409,
                        detail={"detail": "Driver is already assigned in this slot", "code": "driver_busy"},
                    )
                raise HTTPException(
                    status_code=400,
                    detail={"detail": "Invalid slot assignment", "code": "invalid_slot"},
                )
            row.slot_date = sd
            row.start_time = st
            row.end_time = et
    ...
    if "assigned_driver_id" in data:
        ...
        if isinstance(driver_id, str) and not driver_id.strip():
            driver_id = None
    ...
    if (row.slot_date, row.start_time, row.end_time) != old_slot:
        ...
        if cust_email:
            send_booking_rescheduled_email(
                ...
                customer_id=str(row.customer_id) if row.customer_id else None,
                phone=row.phone or None,
                new_end_time=row.end_time or None,
                service_summary=getattr(row, "vehicle_summary", "") or "",
                channel="mobile",
            )
        send_staff_booking_notification(
