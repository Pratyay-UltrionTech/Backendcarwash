...
def _booking_to_dict(db, b: MobileBooking) -> dict[str, Any]:
    ...
    d: dict[str, Any] = {
        ...
        "vehicle_model": (b.vehicle_model or "").strip(),
        "registration_number": (b.registration_number or "").strip(),
...
def list_available_jobs(
    ...
    rows = (
        ...
        .order_by(MobileBooking.created_at.desc())
        .all()
    )
...
def driver_list_leave_requests(
    ...
    rows = (
        ...
        .order_by(MobileDriverLeaveRequest.created_at.desc())
        .all()
    )
...
def driver_submit_leave_request(
    ...
    existing_approved = (
        ...
        .filter(
            ...
            MobileDriverLeaveRequest.leave_date == leave_date,
            MobileDriverLeaveRequest.status == "approved",
        )
        .first()
    )
    if existing_approved:
        raise HTTPException(status_code=400, detail={"detail": "You already have an approved leave for this date", "code": "duplicate_leave"})
