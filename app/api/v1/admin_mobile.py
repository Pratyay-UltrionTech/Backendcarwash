...
def _raise_identity_conflict(field: str, label: str) -> None:
    raise HTTPException(
        status_code=409,
...
def _ensure_unique_mobile_identity(
    db: DbSession,
    *,
    email: str,
    mobile: str,
    ...
    managers = db.query(MobileServiceManager).all()
    drivers = db.query(MobileServiceDriver).all()

    target_email = _normalize_identity_text(email)
    target_mobile = _normalize_phone(mobile)
    target_login = _normalize_identity_text(login_id)

    if target_email:
        for m in managers:
            if exclude_manager_id and m.id == exclude_manager_id:
                continue
            if _normalize_identity_text(m.email) == target_email:
                _raise_identity_conflict("email", "Email")
        for d in drivers:
            if exclude_driver_id and d.id == exclude_driver_id:
                continue
            if _normalize_identity_text(d.email) == target_email:
                _raise_identity_conflict("email", "Email")
    ...
    if target_mobile:
        for m in managers:
            if exclude_manager_id and m.id == exclude_manager_id:
                continue
            if _normalize_phone(m.mobile) == target_mobile:
                _raise_identity_conflict("mobile", "Phone")
        for d in drivers:
            if exclude_driver_id and d.id == exclude_driver_id:
                continue
            if _normalize_phone(d.mobile) == target_mobile:
                _raise_identity_conflict("mobile", "Phone")
    ...
    if target_login:
        for m in managers:
            if exclude_manager_id and m.id == exclude_manager_id:
                continue
            if _normalize_identity_text(m.login_id) == target_login:
                _raise_identity_conflict("login_id", "Login ID")
        for d in drivers:
            if exclude_driver_id and d.id == exclude_driver_id:
                continue
            if _normalize_identity_text(d.login_id) == target_login:
                _raise_identity_conflict("login_id", "Login ID")


def _ensure_unique_mobile_addon_names(items: list[Any]) -> None:
    seen: set[str] = set()
    for item in items:
        normalized = _normalize_item_name(getattr(item, "name", ""))
        if not normalized:
            continue
        if normalized in seen:
            raise HTTPException(
                status_code=409,
                detail={"detail": "Add-on name already used", "code": "conflict", "field": "name"},
            )
        seen.add(normalized)


def _ensure_unique_mobile_service_names(
    db: DbSession, services: list[Any], exclude_vehicle_block_id: str | None = None
) -> None:
    # For replace flows, allow legacy duplicate names that already existed in the same
    # block before this update. This avoids blocking unrelated edits when old data
    # predates the strict uniqueness rule.
    legacy_names_in_same_block: set[str] = set()
    if exclude_vehicle_block_id:
        local_rows = (
            db.query(MobileCatalogServiceItem)
            .filter(MobileCatalogServiceItem.vehicle_block_id == exclude_vehicle_block_id)
            .all()
        )
        legacy_names_in_same_block = {
            _normalize_item_name(row.name)
            for row in local_rows
            if _normalize_item_name(row.name)
        }

    incoming_names: set[str] = set()
    incoming_group_by_name: dict[str, str] = {}
    for svc in services:
        normalized = _normalize_item_name(getattr(svc, "name", ""))
        if not normalized:
            continue
...
def _ensure_unique_mobile_promo_code_name(
    db: DbSession, code_name: str, exclude_id: str | None = None
) -> str:
    ...
    q = db.query(MobilePromotion).filter(
        func.lower(MobilePromotion.code_name) == normalized.lower(),
    )
    if exclude_id:
        q = q.filter(MobilePromotion.id != exclude_id)
    if q.first():
        raise HTTPException(
            status_code=409,
            detail={"detail": "Promo code name already used", "code": "conflict", "field": "code_name"},
        )
    return normalized


def _ensure_unique_mobile_day_rule_title(
    db: DbSession, title: str, exclude_id: str | None = None
) -> str:
    normalized = (title or "").strip()
    if not normalized:
        raise HTTPException(status_code=400, detail={"detail": "Rule title is required", "code": "validation_error"})
    q = db.query(MobileDayTimePriceRule).filter(
        func.lower(MobileDayTimePriceRule.title) == normalized.lower(),
    )
    if exclude_id:
        q = q.filter(MobileDayTimePriceRule.id != exclude_id)
    if q.first():
        raise HTTPException(
            status_code=409,
            detail={"detail": "Day/time pricing title already used", "code": "conflict", "field": "title"},
        )
    return normalized
...
def create_manager(body: MobileManagerCreate, db: DbSession, _admin: AdminUser, request: Request) -> dict[str, Any]:
    ...
    normalized_email = _normalize_identity_text(body.email)
    normalized_login = _normalize_identity_text(body.login_id)
    _ensure_unique_mobile_identity(db, email=normalized_email, mobile=body.mobile, login_id=normalized_login)
...
def update_manager(
    ...
    next_email = _normalize_identity_text(str(data.get("email", row.email) or ""))
    next_mobile = str(data.get("mobile", row.mobile) or "")
    next_login = _normalize_identity_text(str(data.get("login_id", row.login_id) or ""))
    _ensure_unique_mobile_identity(
        db,
        email=next_email,
        mobile=next_mobile,
        login_id=next_login,
    ...
    for key in ("emp_name", "address", "zip_code", "email", "mobile", "doj", "login_id", "active"):
        if key in data:
            ...
            if key == "login_id" and val is not None:
                val = _normalize_identity_text(str(val))
...
def update_driver(
    ...
    if "login_id" in data and data["login_id"] is not None:
        row.login_id = _normalize_identity_text(str(data["login_id"]))
...
def replace_mobile_addons(
    ...
    admin_id = str(_admin["sub"])
    ensure_mobile_global_addons_migrated(db)
    _ensure_unique_mobile_addon_names(body.items)
...
def delete_mobile_addon(addon_id: str, db: DbSession, _admin: AdminUser, request: Request) -> Response:
    started = monotonic_ms()
    admin_id = str(_admin["sub"])
    row = db.query(MobileGlobalAddonItem).filter(MobileGlobalAddonItem.id == addon_id).one_or_none()
    if not row:
        raise HTTPException(status_code=404, detail={"detail": "Mobile add-on not found", "code": "not_found"})
    db.delete(row)
    try:
        db.commit()
    except SQLAlchemyError:
        db.rollback()
        action_log("admin_mobile_delete_addon", "failed", request, addon_id=addon_id, error_code="db_commit_failed")
        raise HTTPException(status_code=500, detail={"detail": "Database operation failed", "code": "db_error"})
    audit_log("admin", admin_id, "delete_mobile_addon", request, addon_id=addon_id)
    action_log("admin_mobile_delete_addon", "success", request, addon_id=addon_id, latency_ms=round(monotonic_ms() - started, 2))
    return Response(status_code=204)
...
def create_vehicle_block(
    ...
    admin_id = str(_admin["sub"])
    ensure_mobile_global_addons_migrated(db)
    _ensure_unique_mobile_service_names(db, body.services)
...
def replace_vehicle_block(
    ...
    _ensure_unique_mobile_service_names(db, body.services, exclude_vehicle_block_id=block.id)
...
def create_promotion(body: MobilePromoIn, db: DbSession, _admin: AdminUser, request: Request) -> dict[str, Any]:
    started = monotonic_ms()
    admin_id = str(_admin["sub"])
    normalized_code_name = _ensure_unique_mobile_promo_code_name(db, body.code_name)
    row = MobilePromotion(
...
def update_promotion(
    ...
    row.code_name = _ensure_unique_mobile_promo_code_name(db, body.code_name, exclude_id=promo_id)
...
def create_day_rule(body: MobileDayTimeRuleIn, db: DbSession, _admin: AdminUser, request: Request) -> dict[str, Any]:
    started = monotonic_ms()
    admin_id = str(_admin["sub"])
    normalized_title = _ensure_unique_mobile_day_rule_title(db, body.title)
...
def update_day_rule(
    ...
    row.title = _ensure_unique_mobile_day_rule_title(db, body.title, exclude_id=rule_id)
