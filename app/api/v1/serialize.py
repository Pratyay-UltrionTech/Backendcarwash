...
def booking_to_dict(b: BranchBooking, db=None) -> dict[str, Any]:
    ...
    cancelled = getattr(b, "cancelled_at", None)
    s0, s1 = slot_service.booking_span_minutes(b.start_time, b.end_time)
    duration_minutes = snap_duration_to_base_slots(s1 - s0)
    addon_ids = loads_json_array(getattr(b, "selected_addon_ids_json", "[]") or "[]")
    addon_names: list[str] = []
    if db and addon_ids:
        from app.models import BranchAddonItem
        catalog_map = {r.id: r.name for r in db.query(CatalogAddonItem).filter(CatalogAddonItem.id.in_(addon_ids)).all()}
        branch_map = {r.id: r.name for r in db.query(BranchAddonItem).filter(BranchAddonItem.id.in_(addon_ids)).all()}
        merged = {**catalog_map, **branch_map}
    ...
    return {
        ...
        "registration_number": getattr(b, "registration_number", ""),
