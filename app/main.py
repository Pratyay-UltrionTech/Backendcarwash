...
def _ensure_booking_reporting_columns() -> None:
    ...
    for table in ["branch_bookings", "mobile_bookings"]:
        ...
        with engine.begin() as conn:
            ...
            if "service_charged_cents" not in cols:
                conn.execute(text(f"ALTER TABLE {table} ADD COLUMN service_charged_cents INTEGER NULL"))
...
def _ensure_admin_accounts_table() -> None:
    """Create admin_accounts table for DB-stored admin users (additive only)."""
    from sqlalchemy import inspect

    insp = inspect(engine)
    if not insp.has_table("admin_accounts"):
        from app.models.admin_account import AdminAccount  # noqa: F401
        AdminAccount.__table__.create(engine)
        _log.info("Created admin_accounts table")
